...
acc.median=matrix(NA,ncol = ncol(acc.oup.dat),nrow=1)
colnames(acc.median)=colnames(acc.oup.dat)
rownames(acc.median)="ALM"
for(ii in 1:ncol(acc.oup.dat)){
  acc.median[1,ii]=median(acc.oup.dat[,ii])
}

saveRDS(acc.median,file="median_ALM.rds")
