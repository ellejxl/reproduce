...
reference <- CreateSeuratObject(counts=as.matrix(t(input[,-1])))
reference <- NormalizeData(reference, normalization.method = "LogNormalize")


train.xx=GetAssayData(reference)
train.xx=as.matrix(train.xx)
celltype=input[,1]
input=data.frame(yy=celltype,t(train.xx))
...
saveRDS(idx.matrix,"processing_BarH_idx.rds")
saveRDS(input,"processing_BarH.rds")
